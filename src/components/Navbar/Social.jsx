...
const Social = () => {
    ...
    const svgs = [<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-github" viewBox="0 0 16 16">
    ...
    const urls = [
        ...
        'https://www.instagram.com/mr_arulkumar_03?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw=='
    ]

    return (
        <div className={`max-mini:transition-all max-mini:duration-100 ${TogglePro ? 'max-mini:w-[310px]' : 'max-mini:w-[300px]'} max-mini:pl-8 max-mini:right-0 w-min max-mini:h-[13%] h-4/6 pt-6 text-white z-[25] absolute bottom-0 right-4 overflow-hidden`}>
            <div className="mini:after:animate-line-ani-h max-mini:after:animate-line-ani-w max-mini:after:right-0 mini:after:bottom-20 mini:h-4/5 mini:justify-center flex mini:flex-col gap-5 relative  after:absolute after:content-[''] after:max-mini:w-[400px] after:max-mini:h-1.5 after:w-1.5 after:h-[500px] after:bg-[#00cba9] after:right-1/2 after:bottom-1/2 after:translate-x-1/2 after:translate-y-1/2">
                ...
                {svgs.map((item, index) => {
                    return (
                        <a key={index} style={{ '--in-dex': index }} target='_blank' href={urls[index]} className='delay-social cursor-pointer hover:bg-[#00cba9] hover:border-red-400 mini:animate-animate-opc-1 max-sm:animate-animate-opc-2 z-30 w-svgWidth-vw border-2 border-[#00cba9] bg-red-500 p-2 rounded-full'>
                            {item}
                        </a>
                    )
                })}
